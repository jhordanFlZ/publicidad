...
def _cleanup_expired(data: dict, ttl_sec: int = DEFAULT_TTL_SEC) -> dict:
    ...
    for name, entry in data.get("profiles", {}).items():
        ...
        if age_sec < ttl_sec:
            ...
            _info(f"Perfil '{name}' expirado hace {int(age_sec)}s, limpiado automaticamente.")
...
def mark_profile_expired(name: str, reason: str = "session_expired") -> None:
    ...
    _save(data)
    _warn(f"Perfil marcado como vencido: '{name}' (reason={reason})")
...
def clear_profile_expired(name: str) -> None:
    ...
    if name in data.get("profiles", {}):
        ...
        _save(data)
        _info(f"Perfil restaurado como activo: '{name}'")
...
def clear_all_expired() -> int:
    ...
    _save(data)
    if count:
        _info(f"Todos los perfiles limpiados ({count} entradas).")
    return count
...
def get_active_profiles(all_profiles: list[str]) -> list[str]:
    ...
    for name in all_profiles:
        if name in expired:
            _info(f"Perfil '{name}' omitido (vencido desde {expired[name].get('date', '?')})")
        else:
            active.append(name)
    return active
...
if __name__ == "__main__":
    ...
    QUIET_MODE = bool(args.best_profile)
